...
describes what users get. Pick a format with `-o`, or let earthsearch choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
earthsearch <command> -o table   # aligned columns for reading
earthsearch <command> -o jsonl   # one JSON object per line, for piping
earthsearch <command> -o json    # a single JSON array
earthsearch <command> -o csv     # spreadsheet friendly
earthsearch <command> -o tsv     # tab-separated
earthsearch <command> -o url     # just the URL column
earthsearch <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
earthsearch <command> --fields id,title,url
```
...
```bash
earthsearch <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
earthsearch <command>            # a table, because this is a terminal
earthsearch <command> | wc -l    # JSONL, because this is a pipe
```
